...
class Client:
    ...
    def login(self):
        ...
        print("正在登陆位于%s:%s的服务器......" % (self.host, self.port))
        while True:
            if times >= 3:
                self.conn.send("exit".encode())
                self.conn.close()
            ...
            if res == "201":
                print("201", settings.RESPONSE_CODE["201"])
                times += 1
                continue
            elif res == "202":
                print("202", settings.RESPONSE_CODE["202"])
                times += 1
                continue
            else:
    ...
    def task_cd(self, cmd_list):
        ...
        json_str = json.dumps(cmd_dict)
        self.conn.sendall(json_str.encode())
    ...
    def task_rm(self, cmd_list):
        if len(cmd_list) == 1:
            print("未指定目录或文件名！")
            return
        choice = input("确认删除？yes 或任意键取消: ").strip()
        if choice != "yes":
            print("放弃任务！")
            return
        else:
        ...
        confirm = self.conn.recv(1024).decode()
        if confirm == "401":
            print("401", settings.RESPONSE_CODE["401"])
        elif confirm == "404":
            print("404", settings.RESPONSE_CODE["404"])
        elif confirm == "311":
    ...
    def task_get(self, cmd_list):
        ...
        if md5_str == file_info["md5"]:
            ...
            os.remove(temp_file)
            confirm = "306"
            print("306", settings.RESPONSE_CODE["306"])
            used_time = end_time - start_time
            size_mb = file_size/1048576
            print("本次任务共传输 %.2f MB数据，用时 %.2f 秒，平均速度 %.2f MB/s"
                  % (size_mb, used_time, size_mb/used_time))
            self.conn.sendall(confirm.encode())
        else:
            confirm = "307"
            print("307", settings.RESPONSE_CODE["307"])
            self.conn.sendall(confirm.encode())
    ...
    def task_put(self, cmd_list):
        ...
        confirm = self.conn.recv(1024).decode()
        if confirm == "400":
            print("400", settings.RESPONSE_CODE["400"])
            return
        if confirm == "401":
            print("401", settings.RESPONSE_CODE["401"])
            return
        ...
        if confirm == "303":
            with open(src_path, "rb") as f:
                ...
                while True:
                    ...
                    if full_size:
                        already = int(transfered_data / full_size * 100)
                        if already == rate+1:
                            self.view_bar(already)
                            rate += 1
        end_time = time.time()
        ...
        if confirm == "306":
            ...
            print("306", settings.RESPONSE_CODE["307"])
